...
탐색 조건에 부합하는 팀의 목록을 반환합니다. 팀 정보 조회 API 와는 달리 할 일 리스트 항목이 생략되며 가입된 팀인지를 나타내는 항목(joined)이 추가됩니다. 팀의 할 일 리스트는 팀에 가입한 팀원들만 조회할 수 있도록 하는 의도입니다.
...
팀의 식별자, 이름, 팀장의 아이디 및 이름, 할 일 리스트 목록, 잠금 상태 등이 반환됩니다. 검색 API 와는 달리 할 일 리스트 목록을 조회할 수 있습니다. 언급했듯이 *팀원이 아니라면* 인가되지 않습니다.
...
요청에 부합하지 않는 할 일 리스트 항목은 생략되어 반환됩니다.
